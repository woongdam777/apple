...
// -> GET / POST 방식을 구분 안하고 모두 받음, 주소만 맞으면 연결
// 2.
...
//	public String login(@RequestParam("inputEmail") String inputEmail) {
	public String login(HttpServletRequest req) {
		...
		// Spring Framework가 제공하는 Argument Resolver(매개 변수 해결사)가 해결해줘서 사용가능
	...
	public String login(/* @RequestParam("inputEmail") */ String inputEmail,
		...
		// 파라미터의 name 속성 값과 매개 변수명이 같으면 @RequestParm 생략 가능!!
		...
		// 작성법 예시 : @RequestParam(value="name", required="fasle", defaultValue="1")
...
//	@PostMapping("/login")
	public String login(/* @ModelAttribute */ Member inputMember) {
		...
		// *** @ModelAttribute 이용해 값이 필드에 세팅된 객체를 "커맨드 객체"라고 한다 ***
...
//		System.out.println(saveId);
		...
		// -> 데이터를 Map(K:V)형식으로 담아 전달
		// -> 기본적으로 request scope를 가진다
		// -> @SessionAttributes 어노테이션과 함께 사용시 Session scope
		...
		// HttpServletResponse resp : 서버 -> 클라이언트 응답방법을 가지고 있는 객체
...
//		System.out.println(loginMember);
		...
		if(loginMember != null) { // 로그인 성공시
			...
			// Servlet -> HttpSession.setAttribute(key, value); -> 이렇게 쓰려면 매개변수로 써서 가져오면됨 하지만 spring에선 다른거 쓴다
			...
			// 응답 객체(HttpServletResponse)를 이용해 만들어진 쿠키를 클라이언트에게 전달
			...
			// HttpServletRequest.getHeader("referer") => spring에서는 더 쉬운방법을 씀 매개변수로 가져온다
		
			/* redirect(재요청)시 기존 요청(request)이 사라지고 새로운 요청(request)을 만들게되어
			 * request된 페이지에서는 이전 요청이 유지되지 않는다!!
			 ...
			 * 
			 * RedirectAttributes : 리다이렉트 시 데이터를 request scope로 전달할 수 있게하는 객체
			 * 
			 * 응답 전 : request scope 리다이렉트시
			 * 응답 중 : session scope 으로 잠시 이동
...
//		session.invalidate(); // 세션 무효화
	...
	public String signUp(Member inputMember, String[] memberAddress, RedirectAttributes ra) {
		...
		// RedirectAttributes ra : 리다이렉트 시 데이터를 request scope으로 전달하는 객체
		// -----------------------------------

		// 만약 주소를 입력하지 않은 경우 빈칸으로 넘어온다 (,,) null로 변경
