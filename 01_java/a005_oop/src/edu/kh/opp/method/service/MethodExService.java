package edu.kh.opp.method.service;

// 기능 제공용 객체를 만들기 위한 클래스
// -> 비즈니스 로직(결과를 도출하기 위해 거친 과정 단, 입력하는 것은 제외)작성
public class MethodExService {
	...
	public int[] fiverandomNumber() {
		...
		// return : 반환, 돌아가다
		// -> 호출한 위치로 돌아감
		// -> 돌아갈 때 가져갈 값이 있으면 return 옆에 작성
		// 매서드 반환형이 void라면 Void methods cannot return a value 오류 뜸
		return arr;
	}
	...
	public String calculate(int num1,int num2,String op) {
        // 문자열(String)  반환
		// 매개변수 명을 일치시켜 이해도 향상 (단, 변수명이 같다고해서 같은 변수는 아니다.(동명이인 개념))
		...
		// 만약에 두번째 숫자가 0이고 연산 기호가 / 또는 %인 경우
