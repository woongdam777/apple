...
import edu.kh.opp.method.service.MethodExService;
...
public class MethodExView {
	...
	public void displayMenu() { // 메뉴 표시(출력) 기능
		...
		do {
			...
			switch(input) {
			case 1 : menu1(); break; // 1 입력시 menu1() 메서드 호출
			// 1 입력->menu1 메서드 호출해서 실행 -> 다시 돌아온다. -> break; 
			case 2 : menu2(); break; // 2 입력시 menu1() 메서드 호출
			case 3 : menu3(); break;
			case 4 : menu4(); break;
	...
	private void menu3() {
		...
		// 1) service에서 fiverandomNumber() 를 호출 -> 로직(난수발생배열)처리완료 -> arr을 가지고 return 돌아가기
		// 2) 난수 5개 저장된 int[]이 반환됨(돌아옴)
		//  -> 반환된 int[]은 실제 배열이 아닌 int[]의 주소값이다. == 얕은 복사 개념 적용
		// 3) 반환된 int[] 주소값을 numbers 참조 변수에 저장
