...
//		for(int i=0 ; i<=arr.length ; i++) { // i = 0,1,2,3,4,5
		...
		// (원인)  arr배열의 인덱스 범위를 초과하는 값까지 
		// 		  증가하도록 작성되어
		// (어디)  실행 시 for문 내 출력 구문에서 
	...
	public void ex6() {
		...
		int max = scoreArr[0];  // 비교하려는 숫자보다 크면 문제가 생김 
								// 1) 매우 작은 수
					 			// 2) 배열의 0번 인덱스 값
	...
	public void ex7() {
		...
		// 1) 초기값을 인덱스 범위에 포함되지 않는 값(음수)을 작성
...
//		}else {
//			System.out.printf("%d번째 있습니다.",index);
//		}
		...
		for(int i=0 ; i<arr.length ; i++) {
			...
			if(input == arr[i]) { // 입력 값과 일치하는 값이 존재하는 경우
				index = i;
				flag = false;
				break; // 일치하는 값을 찾은 경우 더이상 반복하지 않음(성능개선)
			}
		}
	...
	public void ex8() {
		...
		int[] copyArr1 = arr1; // arr1에 저장된 배열의 주소값을 복사
  							   // arr1, copyArr1이 같은 배열을 참조
		...
		// copyArr1 : [9999, 20, 30, 40, 50] (함께 변환)
		...
		// 깊은 복사를 진행할 arr2배열과 같은 크기의 배열을 준비
