...
public class APIService {
	...
	public boolean addStudent2(int grade, int classRoom, int number, String name) {
		...
		for(Student s:studentList) {
			...
			if(s == null) break;
			// s가 참조하는 학생과 temp가 참조하는 학생의 필드 값이 같을 경우(동등한 경우)
//			if(s.equals(temp))	return false;
	 ...
	 */
	public Student[] selectName(String input) { // 김영희
												// 손흥민, 김영희, 박민지 // String.substring으로 하는 방법이 있다.
		
		// String.split(구분자) : 문자열을 구분자를 기준으로 나누어 String[] 형태로 반환
		//
//		String[] names = input.split("/");
		// 무엇을 구분자를 설정할지 인수로 넣는다 여기선 쉼표를 구분자로 input 입력값을 나눠서 String[]배열에 넣는다.
		...
		for(String n : names) {
			for(Student s:studentList) {
				...
				// 입력받은 이름이 학생의 이름과 같다면
				if(s.getName().equals(n)) {
					result[index++] = s; // result 배열에 일치하는 학생 저장 - 주소만 복사, 얕은 복사
				}
			}
		}
	...
	public String printName() {
		...
		// String.join("구분자", String[]) -> String[]배열의 요소를 하나의 문자열로 합침
		//									단, 요소 사이사이에 "구분자" 추가
