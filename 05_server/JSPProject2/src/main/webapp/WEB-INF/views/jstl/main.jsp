...
<body>
    ...
    <pre>
        JSP에서 자주 사용되거나 공통적으로 사용되는 JAVA 코드
        JAVA 코드(if, for, 변수선언, 형변환)를
        스크립틀릿대신 html 태그 형식을 태그화하여
        표준으로 제공하는 라이브러리(if, for 간단히 쓰고 싶으면 이 라이브러리 사용해라)
    </pre>
    ...
    <ol>
        <li>
            <a href="https://tomcat.apache.org/download-taglibs.cgi">JSTP 다운로드 페이지 이동</a>
            <br>
            Standard-1.2.5 -> Jar Files -> Impl, Spec, El 3개 다운로드
        </li>
        <li>
        ...
        <li>
            ...
            taglib JPS 지시자 태그를 추가(2번줄에 작성된 내용)
        </li>
    </ol>
    ...
    <pre>
        - 변수 선언(지정된 scope에 속성 세팅)을 위한 태그를
    ...
    <pre>
        - 변수 제거 : 내장 객체에 세팅된 속성을 제거(remveAttribute("K"))
    ...
    <pre>
        ...
        - 단, escapeXml="ture" (기본값) 설정시 html 태그가 해석 X
            escapeXml="false" : html 태그가 해석 O
    </pre>
