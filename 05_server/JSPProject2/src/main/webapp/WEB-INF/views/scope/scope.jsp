...
<body>
    ...
    <pre>
        ...
        2. request : 최소 2페이지 이상
            -> 요청 받은 페이지(Servlet/JSP)와 이를 위임 받은 페이지(Servlet/JSP)에서 사용 가능

        3. session : 현재 사이트에 접속한 브라우저당 1개씩 생성
                    브라우저가 종료되거나 session 만료될 때까지 유효

        4. application : 하나의 웹 애플리케이션마다 1개씩 생성되는 객체
                        서버가 켜질때 생성되어 종료될 때 없어짐(켜져있는 동안 계속 유지)
        ...
        *******************************
        ...
        EL로 각 scope에 있는 값을 얻어올 수 있다!(방법2가지)
    ...
    <ul>
        ...
        <li>
            request scope <br>
            message : ${requestScope.message} <%-- reqest scope에 저장된 메시지 입니다. --%>
        </li>
        <br>
        <li>
            session scope <br>
