...
// 전역 변수
var str = "JS코드를 함수 내부가 아닌 JS파일 또는 script태그에 바로 작성하면 HTML 랜더링시 바로 수행된다.";
// clg 쓰면 자동완성 됩니다.
console.log(str);
...
// 에러 발생
// console.log("함수 밖에서 선언된 (함)num3 : " + num3);

// JS코드가 중간에 에러가 발생하면 같은 파일/함수 내 에러 이후 부분 코드가 해석되지 않는다.
console.log("함수 밖에서 선언된 (전)num4 : " + num4);
...
function testFn2(){
    ...
    //let3 is not defined 에러
    // console.log("if문내부 let3 : " + let3);
}
...
// var의 호이스팅(hoisting) : 변수가 선언되기 전에 사용가능하게 하는 기술
...
function typeTest(){
    ...
    // boolean
    const inTrue = true;
    box.innerHTML += "<br>inTrue : " + inTrue + " / " + typeof inTrue;
    ...
    console.log(user); // 콘솔로 객체 출력시 브라우저가 파악하기 쉽게 바꿔서 출력
    // 객체 Value 출력 방법(1)
    ...
    // function (* 함수도 자료형이다!! *)
    // function 함수명(){}
    // const sumFn = function(){} /* 해석순서 선언 후에만 쓸수있음 */
...
function doubleFn(fn){ // 전달 받은 함수의 결과를 2배로 반환하는 함수
    ...
    return fn(1,2) * 2; /* 함수의 우선순위를 정할수 있는 방법 */
}
