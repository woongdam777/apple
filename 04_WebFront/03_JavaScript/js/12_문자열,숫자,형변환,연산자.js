...
document.querySelector("#btn2").addEventListener("click", function(e){
    ...
    // 1.25 를 소수점 둘째자리에서 반올림
    // ->1.3
    console.log(Math.round(1.25*10)/10);

    // 부동 소수점(부정확, 용량 작음) -> 고정 소수점(정확, 용량 큼) 변경
    // 숫자.toFixed(보여질 소수 자릿수)
    // - 주어진 숫자를 매개변수에 작성된 숫자 만큼의 소수점 자릿수로 표현될 수 있도록
    ...
    const r = Math.floor(Math.random() * 256); // 0.0 <= x < 256
    const g = Math.floor(Math.random() * 256); // 0.0 <= x < 256
    const b = Math.floor(Math.random() * 256); // 0.0 <= x < 256
