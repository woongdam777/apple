...
document.getElementById("btn2").addEventListener("click", () => {
    ...
    // #test의 마지막 자식 요소의 이전 형제 요소에게 클릭 시 alert()가 출력되는 이벤트 핸들러 추가
    test.lastElementChild.previousElementSibling.addEventListener('click', () => {
        alert("마지막 요소의 이전 형제 요소 입니다.");
    })

    // 다음 형제 요소 선택 : previousElementSibling
    // #test의 마지막 자식 요소의 다음 형제 요소에게 클릭 시 alert()가 출력되는 이벤트 핸들러 추가
...
btn4.addEventListener('click', () => {
    ...
    // -> child를 화면에 추가를 했다고 해서 해당 코드에서 추적을 못하는 것이 아닌 아직 child 변수를 이용해서 계속 접근 가능
