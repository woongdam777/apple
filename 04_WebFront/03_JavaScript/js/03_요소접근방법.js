...
function accessId(){   // 함수 선언 
    ...
    var div1 =document.getElementById("div1"); // div1이라는 변수에 저장

    // 2) div1의 배경색을 변수에 저장
    /* JS에서 CSS 관련 속성을 다룰때는 style을 이용함 */
...
function accessClass(){
    // 함수 정의            class가 같은 요소를 모두 찾아 배열로 반환
    var div2Arr = document.getElementsByClassName("div2");
    // 배열.length : 배열 길이
    // 배열[i] : i번 인덱스 요소
    for(var i =0; i<div2Arr.length; i++){
        div2Arr[i].style.backgroundColor = "rgb(120, 200, "+(i*100)+")";
    }
}
...
function accessTagName(){
    ...
    for(var i = 0 ; i<arr.length; i++){
        arr[i].style.color = arr[i].innerHTML;
    }
}

// css선택자로 접근하기
function cssTest(){
    // querySelector("선택자") : 요소 1개 선택(여러 요소가 존재하면 첫번째 요소 선택)
    // 1개만 선택되는 요소
    document.querySelector("#cssDiv").style.border = " 3px solid pink";

    // 여러 요소 선택(첫번째 요소만 선택 되는지 확인)
