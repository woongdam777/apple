...
// -> null 출력(아직 click했을때 동작할 함수(이벤트 핸들러)가 없음)
document.getElementById('test2a').onclick = function(){
    // function(){} : 익명 함수(이름이 없는 함수)
    //              재사용 목적이 아닌 특정 이벤트에 대한 기능을 만들때 주로 사용
...
test3.addEventListener("click",function(e){
    ...
    // -> 현재 발생한 이벤트에 대한 모든 정보를 담고있는 객체 == 이벤트 객체
...
// for of 구문(향상된 for문) -> 배열(Array), 유사배열(HTMLCollection, NodeList, ...)
...
const boxColorList = document.getElementsByClassName("box"); // 배열
...
for(let i = 0 ; i<colorList.length; i++){
    ...
    // boxColorList[i].style.backgroundColor = e.target.value;
    colorList[i].addEventListener('keyup',function(){
        boxColorList[i].style.backgroundColor = colorList[i].value;
    }); 
}
...
// form태그 기본 이벤트 제거 방법 2 (인라인 이벤트 모델)
function fnCheck(){
    const id = document.querySelector("[name = 'id']").value;
    const pw = document.querySelector("[name = 'pw']").value;
