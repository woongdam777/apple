...
public class EmployeeView {
	...
	public void displayMenu() {
		...
		do {
			try {
				...
				// 사번, 이름, 부서명, 직급명, 전화번호 N행 조회 / 사번 오름차순
				System.out.println("4. 급여 범위로 조회");
				// 사번, 이름, 직급명, 급여 조회 / 급여 내림차순
	...
	private void selectSalary() {
		// 사번, 이름, 직급명, 급여 조회 / 급여 내림차순 / 먼저 입력한 값 이상 / 나중에 입력한 값 이하
		
		System.out.println("\n----- 급여 범위로 사원 조회 -----\n");
		
		System.out.print("조회할 시작 급여를 입력해주세요 : ");
		int input1 = sc.nextInt();
		sc.nextLine();
		
		System.out.print("조회할 마지막 급여를 입력해주세요 : ");
		int input2 = sc.nextInt();
		sc.nextLine();
		
		try {
			List<Employee> empList = service.selectSalary(input1,input2);
			
			if(empList.isEmpty()) {
				System.out.println("[입력된 범위 급여를 받는 사원이 없습니다.]");
				return;
			}
			...
			System.out.println("[급여 범위 사원 조회 중 예외 발생.]");
			e.printStackTrace();
		}
