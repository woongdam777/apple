...
CREATE TABLE "MEMBER"(
	...
	MEMBER_GENDER CHAR(1) CONSTRAINT GENDER_CK CHECK (MEMBER_GENDER IN('M','F')),
	ENROLL_DT DATE DEFAULT SYSDATE NOT NULL,
	UNREGISTER_FLAG CHAR(1) DEFAULT 'N' CONSTRAINT UNREGISTER_CK CHECK(UNREGISTER_FLAG IN ('Y','N')) 
);
...
COMMENT ON COLUMN "MEMBER".UNREGISTER_FLAG IS '탈퇴여부(Y/N)';
...
CREATE TABLE "COMMENT" (
	...
	"CREATE_DTD"	DATE	DEFAULT SYSDATE	NULL,
	"DELETE_FL"	CHAR(1)	DEFAULT 'N'	CHECK("DELETE_FL" IN('Y','N')),
	"MEMBER_NO"	NUMBER	NOT NULL
...
COMMENT ON COLUMN "COMMENT"."CREATE_DTD" IS '댓글 작성일';
...
-- SEQUENCE 생성(MEMBER.BOARD, COMMNET)
...
AND UNREGISTER_FLAG = 'N';

--탈퇴하지 않은 회원 중 아이디 중복 검사
SELECT COUNT(*) FROM "MEMBER" 
WHERE UNREGISTER_FLAG ='N'
...
UPDATE "BOARD"
SET READ_COUNT = READ_COUNT +1 -- 이전 조회 수 +1 값으로 수정
WHERE BOARD_NO =?
;
...
-- 특정 게시글에 대한 댓글 목록 조회
SELECT COMMENT_NO , COMMENT_CONTENT , MEMBER_NO , MEMBER_NM, CREATE_DTD
FROM "COMMENT"
JOIN "MEMBER" USING(MEMBER_NO)
WHERE DELETE_FL = 'N'
AND BOARD_NO = 1
...
-- 댓글 작성
INSERT INTO "COMMENT" 
VALUES(SEQ_COMMENT_NO.NEXTVAL,'댓글내용', DEFAULT, DEFAULT, 1, 1)
;
