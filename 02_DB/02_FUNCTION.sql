...
-- 함수는 SELECT 절, WHERE 절, ORDER BY절, GRUOUP BY절, HAVING절 사용 가능
...
-- 문자열에서 5번째 부터 검색하여 처음 찾는 B의 위치 조회
SELECT  'AABAACAABBAA', INSTR('AABAACAABBAA','B',5) FROM DUAL; -- 9
							-- 123456789

-- 문자열에서 5번째 부터 검색하여 두번째로 찾는 B의 위치 조회
...
-- 소수점 위치 지정 O : 1) 양수 : 지정된 위치의 소수점까지 표현
...
-- SYSTIMESTAMP : SYSDATE 반환 값 + MS 단위 추가
SELECT SYSDATE, SYSTIMESTAMP  FROM DUAL;
--2023-03-07 12:23:19.000	2023-03-07 12:23:19.131 +0900

----------------------------------------------------
-- MONTHS_BETWEEN(날짜, 날짜) : 두 날짜의 개월 수 차이를 반환
SELECT '약 '
		|| ROUND(MONTHS_BETWEEN('2023/07/14','2023/02/09'))
		|| '개월' 훈련기간
FROM DUAL;
...
-- EMPLOYEE 테이블에서 각 사원의 이름, 입사 년, 월, 일 조회 단, 2000년 이상 입사 사원만 조회
 ...
 * 2) 0 : 숫자 한칸을 의미, 오른쪽 정렬, 빈 칸에 0을 추가
...
-- EMPLOYEE 테이블에서 사번, 이름, 연봉 조회(\100,000,000형식으로 조회)
 ...
 * SS   : 초
 * 
 * DAY  : 요일(전체) EX) 월요일, MONDAY 설정값에 따라 다름
 * DY   : 요일(짧게) EX) 월, MON
 * 
 * */
...
/* 날짜 패터 Y, R의 차이점*/
-- 년도를 짧게 해석하는 경우 앞부분에 현재 세기를 적용
-- 50미만 : Y,R둘다 앞부분에 현재 세기(21C == 2000년대)를 적용
-- 50이상 : Y == 현재 세기(2000년대)
--  	   R == 이전 세기(1900년대)
...
-- EMPLOYEE 테이블에서 이름, 그병, 보너스 조회, 급여 * 보너스 조회
/* DB에서 NULL과 연산하는 경우 모든 경과는 NULL */
SELECT EMP_NAME , SALARY , NVL(BONUS,0) , SALARY *NVL(BONUS,0) 
FROM EMPLOYEE; 


-- NVL2(컬럼명, NULL X인 경우 값, NULL O인 경우 값)
-- EMPLOYEE 테이블에서 기존 보너스를 받지 못했던 사원은 0.3으로 변경, 기존에 받았던 사원은 기존 보너스 + 0.2변경
...
-- DECODE(계산식 | 컬럼명, 조건1, 결과1, 조건2, 결과2, 조건3, 결과3.....[,아무것도 일치 안할때])

-- 비교하는 식 또는 컬럼의 값이 일치하는 조건이 있으면 해당 조건의 결과를 반환
-- 만약 일치하는 경우가 없으면 제일 끝 값"[,아무것도 일치 안할때]"을 반환
-- 자바의 switch문과 유사함
...
-- 급여 500만 이상 == '고급'
-- 급여 300만 이상 ~ 500만미만 = '중급'
-- 급여 300만 이하  =='초급'

-- 단, 부서코드가 D6인 사원만 지급코드 오름차순으로 조회
...
SELECT * FROM EMPLOYEE; -- 23row(s) fetched DB에서 23개 데이터를 받아왔다.
SELECT COUNT(*) FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 부사코드가 있는 사원의 수
...
SELECT COUNT(JOB_CODE) FROM EMPLOYEE ; --23
...
-- EMPLOYEE 테이블의 남자 사원 수, 여자 사원 수 조회
SELECT  COUNT(DECODE(SUBSTR(EMP_NO,8,1),'1',1,NULL)) "남자 사원수",
        COUNT(DECODE(SUBSTR(EMP_NO,8,1),'2',1,NULL)) "여자 사원수"
FROM EMPLOYEE;
