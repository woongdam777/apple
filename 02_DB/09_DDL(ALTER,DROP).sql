...
-- ALTER(바꾸다, 변조하다)
-- 수정 가능한 것 : 컬럼[컬럼명, 데이터 타입, 기본값](추가/수정/삭제)
--				, 제약조건(추가/삭제)
--              , 이름변경(테이블, 컬럼, 제약조건)
...
--------------------------------------------------------------------------------
-- 1. 제약조건 추가 / 삭제 -- 제약조건은 수정불가능 -- 수정필요시 삭제 후 다시 추가
...
ALTER TABLE DEPT_COPY ADD CONSTRAINT CHK_DEPT_COPY
...
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C007610; --NOT NULL
-- NOT NULL 제약조건 ADD는 안되는데 DROP은 가능

-- NOT NULL 제거 시 MODIFY 사용
ALTER TABLE DEPT_COPY 
MODIFY DEPT_TITLE CONSTRAINT SYS_C007610 /*NOT*/NULL;
 ...
 * TCL : 트랜잭션 제어 언어{DML 수행 내용을 COMMIT, ROLLBAK, SVAEPOINT*/
...
-- [작성법]
-- DROP TABLE 테이블명 [CASCADE CONSTRAINTS]; -- 종속 제약 조건까지 지우겠다.
