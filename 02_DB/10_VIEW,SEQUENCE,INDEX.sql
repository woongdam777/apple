 ...
 * 
 * - SELECT문의 실행 결과(RESULT SET)를 저장하는 객체 - 조회결과를 저장하는 것 - 테이블모양
 * 
 * 
 * ** VIEW의 사용목적 **
 ...
 * 
 * ** VIEW 사용시 주의 사항 **
 * 1) 가사의 테이블(실체X)이기 때문에 ALTER 구문 사용불가
 * 2) VIEW를 이용한 DML(INSERT,UPDATE,DELETE)이 가능한 경우도 있지만
 * 	  제약이 많이 따르기 때문에 조회(SELECT)용도로 대부분 사용. - 왠만하면 사용하지마!
 * 
 * ** VIEW 작성법 **
 * CREATE [OR REPLACE] [FORCE | NO FORCE] VIEW 뷰이름 [컬럼 별칭]
 * AS 서브쿼리(SELECT문)
 * [WITH CHECK OPTION]
 * [WITH READ ONLY]
 * 
 ...
 * 	 (기본값)NO FORCE(강제X) : 서브쿼리에 사용된 테이블이 존재해야만 뷰 생성
 * 
 * 3) 컬럼 별칭 옵션 : 조회되는 VIEW의 컬럼명을 지정
 * 
 * 4) WITH CHECK OPTION 옵션 : 옵션을 지정한 컬럼의 값을 수정 불가능하게 함.
 * 
 * 5) WITH READ ONLY 옵션 : VIEW에 대해 SELECT만 가능하도록 지정.
 * 
 * */
...
--> SYS 계정으로 접속
ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE; --SQL쉽게 쓸수있게 수정

--> 각자 계정에 CREATE VIEW 권한 부여
GRANT CREATE VIEW TO kh_kjw;

--> 다시 kh_kjw계정으로 접속해서 VIEW 생성 구문 수행
...
--DEPT_COPY2에서 DEPT_ID, LOCATION_ID컬럼만 조회하는 VIEW생성
...
SELECT * FROM V_DCOPY2;
--> VIEW는 값을 저장하지 않는다... 하지만 저장된 것처럼 보인다.
--> 실제로는 VIEW에 저장된 것이 아니라 VIEW를 생성할 때 사용한 서브쿼리 테이블에 추가된다.

-- 원본 테이블(DEPT_COPY2) 확인
SELECT * FROM DEPT_COPY2;
-- D0	null	L3
--> VIEW가아닌 서브쿼리 테이블에 값이 추가됨
--> 원본 테이블에 삽입됨을 확인
-->(중요!) INSERT 구문에 포함되지 않은 DEPT_TITLE컬럼은 NULL이 삽입되어있다.
...
--> 원본 테이블의 DEPT_ID, LOCATION_ID 컬럼에 'D0','L3' 삽입
--> DEPT_TITLE 컬럼에는 NULL 삽입
--  그런데...DEPT_TITLE에는 NOT NULL 제약조건이 설정되어 NULL 값이 삽입될 수 없음.

--SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH_KJW"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다


-- 데이터 무결성 : 중복 X, NULL X -> 신뢰도 높은 데이터
--> 대부분의 컬럼에 NOT NULL 제약조건 추가되어 있음
--> NOT NULL이 설정된 테이블을 이요해서 VIEW를 만들면 INSERT가 거의 불가능함.
...
/* SEQUENCE(순서, 연속)
 * - 순차적으로 일정한 간격의 숫자(번호)를 발생시키는 객체(번호생성기)
 * 
 * ** SEQUENCE 왜 사용할까? **
 * PRIMARY KEY(기본키) : 테이블 내 각 행을 구별하는 식별자 역할, NOT NULL + UNIQUE의 의미를 가짐
 * 
 * 
 * PK가 지정된 컬럼에 삽입된 값을 생성할 때 SEQUENCE를 이용하면 좋다!
 * 
 * [작성법]
...
SELECT * FROM (SELECT TEST_NO FROM TB_TEST ORDER BY TEST_NO DESC) WHERE ROWNUM =1;
...
SET TEST_NO = SEQ_TEST_NO.NEXTVAL,
	TEST_NAME = '고길동' || SEQ_TEST_NO.CURRVAL
WHERE TEST_NO = (SELECT MAX(TEST_NO) FROM TB_TEST); -- 실행안됨
 ...
 * - 인덱스 내부 구조는 B*[BINARY 2진수, BALANCED] 트리 형식으로 되어있음.
 * 
 * ** INDEX의 장점 **
 * - 이진 트리 형식으로 구성되어 자동 정렬 및 검색 속도 증가.
 * - 조회 시 테이블의 전체 낸용을 확인하며 조회하는 것이 아닌
 *   인덱스가 지정된 컬럼만을 이용해서 조회하기 때문에 시스템의 부하가 낮아짐.
 * 
 * ** INDEX의 단점 **
 * - 데이터 변경(INSERT, UPDATE, DELETE)작업시 이진 트리 구조에 변형이 일어남
...
CREATE TABLE TB_INDEX_TEST(
	TEST_NO NUMBER PRIMARY KEY, -- UNIQUE INDEX가 자동 생성
...
BEGIN
	FOR I IN 1..1000000
	LOOP
		INSERT INTO TB_INDEX_TEST VALUES(I,'TEXT' || I);	
	END LOOP;
...
WHERE TEST_ID = 'TEST500000'; --19/15/13/14ms


-- INDEX 사용 O 조회
SELECT * FROM TB_INDEX_TEST
WHERE TEST_NO = 500000; --2/1/0ms

-- TEXT_IN 컬럼에 대한 인덱스 생성
CREATE INDEX IDX_TEST_ID
ON TB_INDEX_TEST(TEST_ID);

--인덱스 생성 후 조회
SELECT * FROM TB_INDEX_TEST
WHERE TEST_IN = 'TEST500000'; --0ms
