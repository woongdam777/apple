...
--SQL Error [918] [42000]: ORA-00918: 열의 정의가 애매합니다 / 테이블명을 앞에 써서 명확하게 하기
...
--					      D1~D9 + NULL / D1~D9    / INNER JOIN은 NULL값 제외 

-- 1) LEFT [OUTER] JOIN  : 합치기에 사용한 두 테이블 중 왼편에 기술된 테이블의 
-- 컬럼 수를 기준으로 JOIN
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE LEFT OUTER JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); --OUTER JOIN은 왼쪽 테이블의 모든 값 포함 / NULL값 포함 
--기존 INNER JOIN 결과에
-- JOIN 구문 왼쪽에 작성된 EMPLOYEE 테이블의 행 중에서 JOIN 결과에 포함되지 않은 행들도 조회하도록 함.
...
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE RIGHT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); --OUTER 생략가능
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음
--> 하나의 테이블을 가지고 생각하지말고 같은 테이블이 2개가 있다고 생각하면 쉬움
...
-- NATURAL JOIN 이 성립되지 않는 경우 -> CROSS JOIN 결과 조회
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE 
...
--*JOIN 순서 중요 !!!*
--> 앞선 두테이블 (EMPLOYEE, DEPARTMENT)의 조인 후 조인된 결과에 LOCATION 테이블을 조인
...
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
-- SQL Error [904] [42000]: ORA-00904: "LOCATION_ID": 부적합한 식별자 // EMPLOYEE 테이블에는 LOCATION_ID가 없음
...
AND JOB_NAME = '대리' AND SUBSTR(LOCAL_NAME,1,4)='ASIA';
