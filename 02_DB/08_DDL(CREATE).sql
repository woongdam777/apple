...
/* 자료형
    NUMBER : 숫자형(정수, 실수)
    CHAR(크기) : 고정길이 문자형 (2000BYTE)  // JAVA에서 char : 문자 하나 /DB에서 CHAR : 문자열(String)
...
-- DEFAULT(기본값) : INSERT/UPDATE 시 해당 컬럼의 값을 작성하지 않거나
--					DEFAULT로 작성하면 DEFAULT 오른쪽에 작성된 값이 컬럼에 반영됨.

-- 만든 테이블 확인
SELECT * FROM MEMBER;
SELECT * FROM USER_TABLES; -->딕셔너리 뷰(데이터 딕셔너리에서 일부만 뽑아 보여주는 가상테이블)
...
INSERT INTO MEMBER
VALUES('MEM03','WE1RW2Q','박철수','950808-1225678', DEFAULT);

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO MEMBER(MEMBER_ID, MEMBER_PWD, MEMBER_NAME, MEMBER_SSN)
VALUES('MEM04','3K32J2Q','이지연','450808-1225678');


--  데이터  삽입 확인
SELECT * FROM MEMBER;
COMMIT; --> TCL
        --> 트랜잭션에 저장된 DML 구문을 DB에 반영
ROLLBACK;
---------------------------------------------------------------
...
INSERT INTO MEMBER2 VALUES('고길동', 9001011234567 , 01045681649);
INSERT INTO MEMBER2 VALUES('김길동', 0001013424427 , 01094381649);

SELECT * FROM MEMBER2;
-- 9001011234567	1045681649
-- 1013424427	1094381649
--> NUMBER 타입의 컬럼은 제일 앞의 값 0을 유지하지 않는 문제 발생
--> 문제 해결 방법 : CHAR, VARCHAR2 타입으로 컬럼 타입을 지정
...
-- SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH_KJW"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다.
...
--    USER_ID VARCHAR2(20) CONSTRAINT USER_ID_UK UNIQUE, -- 컬럼 레벨 제약조건 설정(이름 지정)
    USER_ID VARCHAR2(20),
...
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KJW.SYS_C007522)에 위배됩니다
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KJW.USER_ID_UK)에 위배됩니다
...
-- 테이블에 대한 식별자(IDENTIFIER) 역할을 함 / 각 행을 구분하는 역할
-- NOT NULL + UNIQUE 제약조건의 의미 만 같을뿐 같은건 아니다!!!
...
--    USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY,  -- 컬럼 레벨(제약조건명 O) 권장 WHY? 오류뜰때 원인 찾기 쉬움
...
--> 기본키 중복으로 오류
--SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KJW.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
--SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH_KJW"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
--SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KJW.PK_USERNO_USERID)에 위배됩니다
...
--SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH_KJW"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
--복합키로 PK가 설정되어 있어도 결국 PK ==> NULL 삽입 불가능
...
--> NULL 사용 가능.(외부 테이블에 참조할 값이 없다, 참조X / JAVA에서 NULL과 비슷한 의미)
...
--SQL Error [2291] [23000]: ORA-02291: 무결성 제약조건(KH_KJW.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
-- 값을 제공해주는 테이블을 부모, 값을 가져다 쓰는 테이블을 자식이라 한다. 그래서 USER_GRADE가 부모고 USER_USED_FK가 자식이다.
...
WHERE GRADE_CODE = 20;
-- GRADE_CODE = 20번 행은 자식 테이블에서 참조하고 있지 않으므로 삭제가 제한되지 않음.

-- GRADE_CODE 중 10은 외래키로 참조되고 있으면 삭제가 불가능함.
DELETE FROM USER_GRADE
WHERE GRADE_CODE = 10;
-- SQL Error [2292] [23000]: ORA-02292: 무결성 제약조건(KH_KJW.GRADE_CODE_FK1)이 위배되었습니다- 자식 레코드(기록)가 발견되었습니다
...
CREATE TABLE USER_USED_FK3(
  ...
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK3
  					REFERENCES USER_GRADE3 --부모 테이블 컬럼 미작성시 PK로 자동연결
  					 ON DELETE CASCADE
);
...
-- SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(KH_KJW.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST(
	USER_NO VARCHAR2(10) CONSTRAINT PK_USER_TEST PRIMARY KEY, 
    USER_ID VARCHAR2(10) CONSTRAINT UK_USER_ID UNIQUE,
    USER_PWD VARCHAR2(10) CONSTRAINT NN_USER_PWD NOT NULL,
    PNO VARCHAR2(14) CONSTRAINT UK_PNO NOT NULL CONSTRAINT NN_PNO UNIQUE,
    GENDER VARCHAR2(6) CONSTRAINT CK_GENDER CHECK (GENDER IN('남','여')),
	PHONE VARCHAR2(13),
...
VALUES (1, 'user01', 'pass01', '880122-1234567', '남', '010-1431-9599', '서울시 강남구 역삼동', 'N');
INSERT INTO USER_TEST
VALUES (2, 'user02', 'pass02', '880122-3234567', '여', '010-1431-9599', '경기도 도봉시 인사동', 'N');
INSERT INTO USER_TEST
VALUES (3, 'user03', 'pass03', '120122-1234567', '여', '010-1431-9599', '부산시 창원구 역삼동', 'N');
INSERT INTO USER_TEST
VALUES (4, 'user04', 'pass04', '330122-1234567', '남', '010-1431-9599', '내가시 강남구 역삼동', 'N');
INSERT INTO USER_TEST
VALUES (5, 'user05', 'pass05', '440122-8834567', '남', '010-1431-9599', '누구시 강동구 역삼동', 'N');
