 ...
 * 
 * - RESULT SET은 0개 이상의 행이 포함될 수 잇다.
...
-- 컬럼 값 : 행과 열이 교차되는 테이블의 한칸(==한 셀)에 작성된 값(DATA)
...
-- 별칭 지정 방법
-- 1) 컬럼명 AS 별칭 : 문자OK,띄어쓰기NO,특수문자NO
-- 2) 컬럼명 AS "별칭" : 문자OK,띄어쓰기OK,특수문자OK
-- 3) 컬럼명 별칭 : 문자OK,띄어쓰기NO,특수문자NO
...
/*해석 순서 1.어떤 테이블에서 2.어떤 조건에 해당하는 행을 거르고 3.어떤 것을 조회할 열을 조회함*/
...
-- EMPLOYEE 테이블에서 급여가 300만 이상 500만 이하인 직원의 모든 정보 조회
...
-- EMPLOYEE 테이블에서 급여가 300만 이상 500만 이하가 아닌 직원의 모든 정보 조회

SELECT *
FROM EMPLOYEE
WHERE SALARY < 3000000 OR SALARY > 5000000;

-- 컬럼명 NOT BETWEEN A AND B :  컬럼 값이 A이상 B이하가 아닌 경우 TRUE == A미만 또는 B초과

SELECT *
FROM EMPLOYEE
WHERE SALARY NOT BETWEEN 3000000 AND 5000000;
--WHERE NOT SALARY BETWEEN 3000000 AND 5000000; -이것도 가능
...
-- 컬럼명 IN (값1, 값2, 값3, .....) : 컬럽 값에 ()안의 값이 포함된 행은 TRUE

SELECT EMP_ID 사번,EMP_NAME 이름, DEPT_CODE 부서코드
FROM EMPLOYEE
WHERE DEPT_CODE IN ('D5','D6','D9');

-- EMPLOYEE 테이블에서 부서코드가 'D5' 또는 'D6'또는 'D9'가 아닌 사원의 사번, 이름, 부서코드 조회
...
--'' : 값, 리터럴
--"" : 계정명, 비밀번호, 컬럼명, 테이블명 / 값이 아닌 것들에 대한 대소문자 구분
 ...
 * - LIKE 패턴(= 와일드 카드) 2종류
 * '%'(포함)
 * - '%A' : 문자열이 앞은 어떤 문자든 포함되고 마지막은 A -> A로 끝나는 문자열
 * - 'A%' : A로 시작하는 문자열
 * - '%A% : A가 포함된 문자열
 * 
 * '_' (글자 수)
...
SELECT DEPT_CODE 사번, EMP_NAME 이름
FROM EMPLOYEE
...
SELECT DEPT_CODE 사번, EMP_NAME 이름, PHONE 전화번호
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 이일일에 @ 앞글자가 5글자인 사원의 사번, 이름, 이메일 조회

SELECT DEPT_CODE 사번, EMP_NAME 이름, EMAIL 이메일
FROM EMPLOYEE
WHERE EMAIL LIKE '_____@%';

-- EMPLOYEE 테이블에서 이베일에 @ 앞글자가 3글자인 사원의 사번, 이름, 이메일 조회

SELECT DEPT_CODE 사번, EMP_NAME 이름, EMAIL 이메일
FROM EMPLOYEE
WHERE EMAIL LIKE '____%';
--문제점 : 와일드 카드 문자(_)와 패턴에 사용된 일반문자가 동일하여 구분이 안되는 문제발생
...
--> '1990/01/01' ==문자열
--> 오라클 DB는 작성된 값이 다른 형식의 데이터 타입이여도 표기법이 다른 데이터 타입과 일치하다면
 ...
 * 1: FROM 테이블명
 * 2: WHERE 조건식
 * 4: ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ 오름/내림 차순] [NULLS FIRST | LAST]
 * 
 * */
...
SELECT EMP_NAME , SALARY 
FROM EMPLOYEE 
ORDER BY SALARY /*ASC*/; --> 오름차순(ASC)가 기본값 

-- EMPLOYEE 테이블에서 모든 사원의 이름, 급여를 급여 내림차순으로 조회

SELECT EMP_NAME , SALARY 
FROM EMPLOYEE 
ORDER BY SALARY DESC; --> 내림차순을 원할경우 DESC 작성 
...
-- 해석 순서상의 오류 발생(SELECT절 해석이 안된 상태에서 별칭을 WHERE절 작성)
...
/* <정렬 중첩>
 * - 큰 분류를 먼저 정렬하고, 내분류를 다음에 정렬하는 방식
 * */

-- 부서 코드별 급여 내림차순 (부서코드는 내림차순)
