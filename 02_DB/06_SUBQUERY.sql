...
FROM EMPLOYEE 
WHERE DEPT_CODE = 'D9'; -- T선동일, 송종기, 노옹철
...
-- 급여가 가장 적은 사람의 사번
-- ROWNUM : 행 번호를 출력하는 가상의 컬럼\
-- 주의사항!! : ORDER BY절 해석 전에 먼저 행번호를 부여 [정렬이 되기전에 번호가 붙는다]
SELECT /*ROWNUM*/ EMP_ID
FROM (SELECT EMP_ID FROM EMPLOYEE ORDER BY SALARY) --RESULT SET 표(TABLE취급)
WHERE ROWNUM = 1;
...
-- SELECT 절에 작성된 그룹 함수 이외의 컬럼을 모두 GROUP BY절에 작성해 그룹이 구성된다!!
-- GROUP BY 절에 작성된 컬럼명 이외의 컬럼들은 그룹으로 묶이지 않는다.
-------------------------------------------------------------------------
...
FROM EMPLOYEE
WHERE MANAGER_ID IS NOT NULL; --7행 중복제거
...
SELECT EMP_ID ,EMP_NAME ,DEPT_CODE , JOB_NAME, '사수' AS 구분
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE EMP_ID IN (SELECT DISTINCT MANAGER_ID  FROM EMPLOYEE WHERE MANAGER_ID IS NOT NULL); --6행

-- 4) 일반 직원에 해당하는 사원들 정보 조회 (이때, 구분은 '사원'으로)
SELECT EMP_ID ,EMP_NAME ,DEPT_CODE , JOB_NAME, '사원' AS 구분
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE EMP_ID NOT IN (SELECT DISTINCT MANAGER_ID  FROM EMPLOYEE WHERE MANAGER_ID IS NOT NULL); --17행
...
--선택 함수(DECODE, CASE 중 CASE를 써본다.)로 풀이
SELECT EMP_ID ,EMP_NAME ,DEPT_CODE , JOB_NAME,
	CASE
...
AND SALARY > ANY(SELECT SALARY FROM EMPLOYEE
				JOIN JOB USING(JOB_CODE) WHERE JOB_NAME ='과장');
-- > ANY 가장 작은 값보다 큰 값  / < ANY 가장 큰 값보다 작은 값
-- 조건들중 하나만 성립되면 OK
...
AND SALARY > ALL(SELECT SALARY FROM EMPLOYEE
					NATURAL JOIN JOB WHERE JOB_NAME = '차장') ;
-- 모든 조건이 성립되야됨
...
WHERE DEPT_CODE IN (SELECT  DEPT_ID FROM DEPARTMENT 
					WHERE LOCATION_ID = (SELECT LOCAL_CODE FROM LOCATION
					WHERE NATIONAL_CODE = 'KO')); -- 한국에서 근무하는 사원 조회
...
SELECT EMP_NO, EMP_NAME, DEPT_CODE, DEPT_TITLE, JOB_CODE, JOB_NAME 
FROM EMPLOYEE
NATURAL JOIN JOB
JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID)
WHERE EMP_NAME != '노옹철'
...
--				   GROUP BY DEPT_CODE) --신뢰도 떨어짐
	
--WHERE (DEPT_CODE,HIRE_DATE) IN(SELECT DEPT_CODE ,MIN(HIRE_DATE)
--				   FROM EMPLOYEE WHERE ENT_YN ='N'
--				   GROUP BY DEPT_CODE) -- 정확하지만 NULL값 판단하기 위한 서브쿼리가 필요해서 길어짐

--WHERE HIRE_DATE = (SELECT MIN(HIRE_DATE)
--					FROM EMPLOYEE SUB
--					WHERE (ENT_YN = 'N' AND SUB.DEPT_CODE = MAIN.DEPT_CODE) --NULL아닌 경우
...
SELECT EMP_NAME , JOB_CODE , SALARY ,
	...
	WHERE SUB.JOB_CODE = MAIN.JOB_CODE) "직급의 급여 평균"
FROM EMPLOYEE MAIN;
...
SELECT EMP_ID 사번, EMP_NAME 이름, MANAGER_ID 관리자사번,
		NVL((SELECT EMP_NAME FROM EMPLOYEE SUB
			WHERE SUB.EMP_ID = MAIN.MANAGER_ID),'없음') 관리자명
FROM EMPLOYEE MAIN;

--셀프 조인
SELECT MAIN.EMP_ID 사번, MAIN.EMP_NAME 이름, MAIN.MANAGER_ID 관리자사번,NVL(SUB.EMP_NAME,'없음') 관리자명
FROM EMPLOYEE MAIN
...
-- 조건절에 ROWNUM을 작성하는 경우 반드시 조건 범위 내에 1(1번 행)이 포함되어야 한다.
...
-- 문제 원인 : SELECT 해석 순서 상 ORDER BY가 가장 마지막으로 해석되기 때문에
...
-- 해결 2) 해결 1의 조회 결과를 FROM 절에 작성하여 상위 5행만 조회
SELECT ROWNUM, EMP_NAME, SALARY
FROM (SELECT EMP_NAME ,SALARY FROM EMPLOYEE ORDER BY SALARY DESC) --> 메인쿼리에 포함된 VIEW 생성구문 = 인라인뷰
WHERE ROWNUM <=5;
...
-- ** 인라인뷰 사용시 컬럼명에 유의해야한다! **
	--> 메인쿼리에서 인라인 뷰의 컬럼명을 그대로 사용해야 한다!!
-- 급여 평균이 590만인 부서를 조회
/*3*/SELECT * 
/*1*/FROM (SELECT DEPT_CODE 부서코드, DEPT_TITLE 부서명, FLOOR(AVG(SALARY)) 평균급여
	  FROM EMPLOYEE
	  LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
	  GROUP BY DEPT_CODE, DEPT_TITLE
	  ORDER BY 평균급여 DESC)
/*2*/WHERE 평균급여 = 5900000;
...
/*0*/WITH TOP_SALARY AS (SELECT EMP_NAME, SALARY FROM EMPLOYEE ORDER BY SALARY DESC)
...
-- 급여를 많이 받는 순서로 조회
/*3*/SELECT  EMP_NAME , SALARY, RANK() OVER(ORDER BY SALARY DESC) 순위
/*1*/FROM EMPLOYEE; 
--/*2*/WHERE RANK() OVER(ORDER BY SALARY DESC) <=5;
--SQL Error [30483] [99999]: ORA-30483: 윈도우 함수를 여기에 사용할 수 없습니다
--SELECT 절에서만 쓸수있다. 쓰고 싶으면 서브쿼리 이용
SELECT * FROM 
