	...
	public void displayMenu() {
		...
		do {
			try {
				...
				sc.nextLine(); // 입력 버퍼에 잘 못 입력된 내용 제고
...
//			System.out.println(s.toString());
			...
			// 자동으로 .toString()를 붙여서 호출 (컴파일러가 해준다)
	...
	/** 학생 전체 정보를 입력 받아 Student 객체로 반호나하는 메서드
	 * @return std:Student
	 */
...
//		if(list.size() == 0) { // 저장된 객체의 수가 0개인 경우
		if( list.isEmpty() ) { // 리스트에 저장된 객체가 없을 경우
			...
			for(Student s : list) System.out.println(s); // print구문에선 컴파일러가 자동으로 s.toString 붙여준다.
		}
	...
	private void selectGender() {
		 ...
		 * [M 또는 F만 입력 해주세요.] 
		 * 
		 * 조회할 성별을 입력하세요(M/F) : (정상 입력이 될때까지 반복)
		 * 
		 * *** 정상 입력하는 경우 ***
		 * 조회할 성별을 입력하세요(M/F) : F
		 * [여학생 목록]
		...
		while(true) { // 무한 반복
			...
			System.out.println("조회할 성별을 입력하세요(M/F) : ");
...
//			char input = sc.next().charAt(0); sc.nextLine();
//			Character.toUpperCase(input);
//			if(input=='M' || input=='F') {
//				List<Student> list = service.selectGender(input);
//				if(input == 'M') {
//					System.out.print("[남학생 목록]");
//				}else {
//					System.out.print("[여학생 목록]");
//				}
//				System.out.println();
//				for(Student s : list) {
//					System.out.printf("%d학년 %d반 %d번 %s(%c)\n",s.getGrade(),s.getClassRoom(),s.getNumber(),s.getName(),s.getGender());
