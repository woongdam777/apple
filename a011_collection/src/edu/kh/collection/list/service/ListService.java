...
public class ListService {
	 ...
	 * -> ArrayList(최신), Vector(구식), LinkedList
	 * */
...
//		ArrayList list = new ArrayList(); // 기본 생성자 -> 10칸짜리 생성
		...
		// -> 예상되는 데이터 수에 따라 생성자를 골라서 사용(생성자에 따라 속도, 메모리 효율이 달라짐)
		// -> 크기를 예상하기 어려울 경우 기본생성자 쓰자!
...
//		list.add(10,"마지막 삽입");  java.lang.IndexOutOfBoundsException 오류발생
		...
		for(int i=0 ; i<list.size();i++) {
			...
			// - 각 요소마다 타입이 다르기 때문에 원하는 코드가 있을 경우
			//	타입검사 + 다운 캐스팅이 강제됨
			
			
			if(list.get(i) instanceof String) { // 요소가 String인 경구
				char ch = ((String)list.get(i)).charAt(0);
				System.out.println("ch : " + ch);
			} else if(list.get(i) instanceof Integer) { //요소가 Integer인 경구
