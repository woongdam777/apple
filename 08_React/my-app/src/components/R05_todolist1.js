import React, { useState } from 'react'; //jmrs
...
const InputName = () => {
    ...
    // useState('') : name 변수에 대입되는 초기값을 ('')빈칸으로 지정
...
const TodoList1 = () => {
    ...
    const InputTodo = () => {
        ...
        const addTodo = () => {
            ...
            // todos에 newTodo를 추가한 객체배열 newTodos생성
            ...
            */}
            <ul>
                {todos.map( (todo, index) =>{
                    return(
                        /* key 속성 : 배열(list) 출력 시 요소를 구분하는 key값 - 리렌더링를 할때 이전값과 변경된 값을 구분하기 위해...변경된게 없으면 재출력안함 성능적개선 */
                        <li key={index}>
                            <input type='checkbox'
...
// 외부에서 해당 파일 import 시 TodoList1 함수를 기본값으로 내보내는 기본값으로 지정
