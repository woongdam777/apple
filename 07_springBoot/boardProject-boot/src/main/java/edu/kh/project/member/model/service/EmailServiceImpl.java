...
public class EmailServiceImpl implements EmailService {
	...
	private String fromEmail = "woongdam777@gmail.com";
	...
	public int signUp(String email, String title) {
		...
		try {
			...
			// 메일 내용 - 이미지 주소를 입력하면 이미지도 전송된다!
			String mailContent = "<p>Board Project " + title
		...
		int result = dao.updateAuthKey(map); // 값이 없으면 0 그러면 아래 if문 실행 + 시간도 저장(DB관리하는 스케줄러)

		if (result == 0) {
			result = dao.insertAuthKey(map); // 새로운 값 추가
		}
