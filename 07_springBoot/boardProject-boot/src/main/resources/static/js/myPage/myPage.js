...
if(imageInput != null){ // 화면에 imageInput이 있는 경우

    // 프로필 이미지가 출력되는 img태그의 src 속성을 저장
    originalImage = profileImage.getAttribute("src");

    // 회원 프로필 화면 진입 시 현재 회원의 프로필 이미지 상태 확인
    ...
    imageInput.addEventListener("change", e => {
        ...
        if(file == undefined){ // 파일을 한번 선택한 후 취소했을 때
        ...
        if(file.size > maxSize){ // 파일의 크기가 최대 크기를 초과한 경우
            alert("2MB 이하의 이미지를 선택해주세요.")
            imageInput.value = "";
            // input type="file" 태그에 대입할 수 잇는 value는 "" (빈칸) 뿐이다
            deleteCheck = -1 ; // 취소 파일 없음 == 초기상태
    ...
    document.getElementById("profileFrm").addEventListener("submit", e => {
        // initCheck : 초기 프로필 이미지 상태를 저장하는 변수
        //           false == 기본이미지 true == 이전 업로드 이미지
        // deleteCheck : 프로필 이미지가 새로 업로드 되거나 삭제 되었음을 나타내는 변수 
        //               -1 == 초기값, 0 == 프로필 삭제(X버튼), 1 == 새 이미지 업로드
