...
/* 정규 표현식(Regular Expression)
    ...
    *** 정규표현식 객체가 제공하는 메서드(함수) ***
    1. regEx.test(문자열)
        -> 문자열이 정규표현식 패턴에 부합하면 true, 아니면 false 반환
    2. regEx.exec(문자열)
        -> 문자열이 정규표현식 패턴에 부합하면
            첫 번째 매칭되는 문자열을 반환 없으면 null반환
    ...
    *** 정규 표현식의 메타 문자***
    ...
    [가-힣] : ㄱ부터 힣까지  모든 한글(자음만, 모음만 있는 경우 제외)
    ...
    if(regEx.test(memberEmail.value)){ // 유효한 경우
        ...
        .then(count => {
            ...
            if(count == 0){
                ...
                emailMessage.classList.remove("confrim"); // .confrim 스타일 제거
        ...
        emailMessage.innerText="이메일 형식이 유효하지 않은 형식 입니다";
        emailMessage.classList.add("error"); // .error스타일 적용
        emailMessage.classList.remove("confirm"); // .confirm스타일 제거
        checkObj.memberEmail = false;  // 유효 X
    }
})
...
memberPw.addEventListener("input", () => {
    ...
    if(memberPw.value.trim().length == 0){
        ...
        pwMessage.classList.remove("confirm", "error"); // 검정글씨로 바꾸기
        checkObj.memberPw = false; // 빈칸 == 유효X
        return;
    }
    ...
    if(regEx.test(memberPw.value)){
        ...
        // 비밀번호가 유효하게 작성된 상태에서 비밀번호 확인이 입력되지 않았을때
        if(memberPwConfirm.value.trim().length == 0){
            pwMessage.innerText = "유효한 비밀번호 형식입니다";
            pwMessage.classList.add("confirm");
            pwMessage.classList.remove("error");
        }else{  // 비밀번호가 유효하게 작성된 상태에서 비밀번호 확인이 입력되어 있을때
...
memberPwConfirm.addEventListener('input', () => {
    ...
    if(checkObj.memberPw){ // 비밀번호가 유효하게 작성된 경우에 비교
        // 비밀번호 == 비밀번호 확인 (같은 경우)
        if(memberPw.value == memberPwConfirm.value){
            pwMessage.innerText = "비밀번호가 일치합니다";
            pwMessage.classList.add("confirm");
...
sendAuthKeyBtn.addEventListener("click", function(){
    ...
    if(checkObj.memberEmail){ // 중복이 아닌 이메일인 경우
        ...
        authTimer = window.setInterval(()=>{ // 인증시간이 점점 감소하게
...
checkAuthKeyBtn.addEventListener("click", function(){
    ...
    if(authMin > 0 || authSec > 0){ // 시간 제한이 지나지 않은 경우에만 인증번호 검사 진행
        /* fetch API */
        const obj = {"inputKey":authKey.value, "email":tempEmail} // JS객체 만들기
        const query = new URLSearchParams(obj).toString()
        // URLSearchParams : inputKey=123456&email=user01  파라미터가 많을때 쿼리스트링형식으로 자동변환해준다!!

        fetch("/sendEmail/checkAuthKey?" + query)
        .then(resp => resp.text())
        .then(result => {
            if(result > 0){
                clearInterval(authTimer); // 인증시간 멈추기
...
document.getElementById("signUpFrm").addEventListener("submit", e=>{
    ...
    for(let key in checkObj){
        if(!checkObj[key]){ // 각 key에 대한 value(true/false)를 얻어와
            ...
            case "memberPwConfirm" : alert("비밀번호가 일치하지 않습니다"); break;
            case "memberNickname" : alert("닉네임이 유효하지 않습니다"); break;
            }
            // 유효하지 않은 input 태그로 focus 이동
            // key 를 input의 id 와 똑같이 설정
