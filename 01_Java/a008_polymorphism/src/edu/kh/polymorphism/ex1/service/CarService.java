...
public class CarService {
	...
	// - 상속을 이용한 기술로 부모 타입의 참조 변수 하나로
	// 	 여러 타입의 자식 객체를 참조할 수 있다.
	
	public void ex1() {
		
		// 부모 참조 변수 = 부모 객체 , 부모가 부모를 참조하는 형태
		Car c1 = new Car();
		...
		// Truck의 부모가 Car, Truck안에 Car가 포함되어 있고
		// c2는 Truck안에 있는 Car만 참조
		c2.setWheel(10);
		c2.setSeat(3);
		c2.setFuel("경유");
		...
		// c2.setWeight(2.5); c2로 Truck 필드 접근 불가능하여 에러
	...
	public void ex3() {
		...
		if( c instanceof Truck){
			...
			// c가 참조하는 객체가 LightCar 객체인 경우
			System.out.println("할인율 : " + ((LightCar)c).getDiscountOffer() *100+"%");
			
		}else {
			// c가 참조하는 객체가 Truck, LightCar 객체가 아닌 경우
...
//			printCar(Car객체 주소 또는 Car를 상속받은 객체 주소);
	...
	public void printCar(Car c) {
		...
		// 바인딩 : 메서드 호출 구문과 수행될 메서드를 연결 하는 것(묶는 것)
		...
		// 동적 바인딩
		// - "프로그램 실행 중" 실행할 당시의 객체의 자료형을 기준으로
